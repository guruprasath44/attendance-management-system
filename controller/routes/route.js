...
router.get('/admin/add_subject', (req, res) => {
  res.render('add_subject', { userData: username, success: '' });
});

router.get('/student/subject_info', (req, res) => {
  res.render('subject_info', { userData: username, subjectData: '', sem: '' });
});
// router.get('/student/student_profile',function(req,res){
//     res.render( "student_profile",{name: username,success:''});
// })
...
router.get('/admin/mark_attendance', (req, res) => {
  res.render('mark_attendance', { userData: username, success: '' });
});
...
router.put('/attendance/report/generate', (req, res) => {
  const { semester } = req.body;
  const sub = req.body.sub_code;
  const { time } = req.body;
  const { date } = req.body;
...
router.post('/user/login', (req, res) => {
  ...
  // else{
  //     res.render("Login",{er:'Please try Again'});
  //  else{
  //     res.render('Incorrect details');
  //     // res.end();
  // }
});
  ...
  if (re.test(`${email}`)) {
    ...
    connection.query('insert into faculty_signup values(?,?,?)', [name, email, pass], (err, results) => {
      ...
      if (name.length != 0) {
        console.log('Values Inserted');
        setTimeout(() => {
          res.render('login', { userData: username });
        }, 2000);
      } else {
        setTimeout(() => {
          res.render('register');
